...
wake_cars
```
Roughly the same percentage of zero car households in food deserts (3.89%) compared to in all of Wake County (3.96%).

First check -> Got the same answer using filter
```{r}
wake|>
  filter(near_store == 0) |>
  summarize(pct_no_cars = sum(zero_vehicle_households)/sum(total_households) * 100)

```
Second check -> same answer
```{r}
wake|>
  summarize(pct_no_car = sum(zero_vehicle_households)/sum(total_households) * 100)
```
...
wake |>
  summarise(
    income_fd = sum(households_income_less_than_35k[near_store==0])/sum(total_households[near_store==0]) * 100,
    income_all = sum(households_income_less_than_35k)/sum(total_households) * 100
  )
```
Slightly higher percentage of low-income households living in food deserts (21.46%) compared to in all of Wake County (18.80%).
